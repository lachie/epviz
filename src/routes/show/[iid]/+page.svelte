<script lang="ts">
  ...
  const zoomForLabel = (label: string): string =>
    zooms[label as Zoom] || zooms["100%"];
  ...
  $effect(() => {
    if (data.iid) {
      fetch(`/api/track`, {
        method: "POST",
        body: JSON.stringify({ iid: data.iid }),
        headers: {
    ...
    <div class="flex flex-row gap-x-4">
      ...
      <div class="flex flex-row">
        <form
          method="POST"
          class=""
          action={`?/${data.bookmarked ? "un" : ""}bookmark`}
          use:enhance
        >
          <button class="btn btn-primary btn-sm font-bold" type="submit">
            {data.bookmarked ? " - " : " + "}
          </button>
        </form>
        <form
          method="POST"
          class=""
          action={`?/${data.favourited ? "un" : ""}fave`}
          use:enhance
        >
      ...
      <div>
        {#each Object.keys(zooms) as zoomLabel}
          <button
            class:underline={zoomLabel === $zoom}
            onclick={() => setZoom(zoomLabel)}>{zoomLabel}</button
          >
        {/each}
      </div>
    </div>
